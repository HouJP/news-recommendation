...
import com.houjp.common.{Log, FileOpts}
...
object IO {
  ...
  def loadDocs(sc: SparkContext, sql_c: SQLContext, date: String): Seq[(String, Seq[String])] = {
    val fp = s"${com.houjp.recommendation.evt_docs_pt}/$date.txt"
  ...
  def loadUserView(sc: SparkContext, date: String): RDD[(String, String)] = {
    val fp = s"${com.houjp.recommendation.evt_user_pt}/$date.txt"
  ...
  def loadUserInfo(sc: SparkContext, date: String): RDD[(String, Map[String, Double])] = {
    val fp = s"${com.houjp.recommendation.evt_out_pt}/$date.user.obj"
  ...
  def saveUserInfo(user_info: RDD[(String, Map[String, Double])], date: String): Unit = {
    user_info.saveAsObjectFile(s"${com.houjp.recommendation.evt_out_pt}/$date.user.obj")
  }

  def loadRecDocs(sc: SparkContext, date: String): RDD[(String, String)] = {
    val fp = s"${com.houjp.recommendation.evt_out_pt}/$date.docs.obj"
  ...
  def saveRecDocs(rec_docs: RDD[(String, String)], date: String): Unit = {
    rec_docs.saveAsObjectFile(s"${com.houjp.recommendation.evt_out_pt}/$date.docs.obj")
  }
}